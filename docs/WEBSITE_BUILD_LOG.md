...
Formal German and Spanish address restored. Instructions now describe this single product rather than multiple selected products. Setup help uses the full corporate contact URL. Command-window instructions are explicit even when launcher setup is skipped; ChatGPT field-name variation is disclosed. Windows and Microsoft Word requirements are stated beside the computer selector. No product runtime, bundle, payment or licensing files changed. The prior entry describes the combined five-page pass; storefront-specific edits apply only to that repository. Revised checks and candidate verification are recorded in MCP Servers/Codex/scratch/install-ux-20260914. No desktop app installation is certified by these web checks.
